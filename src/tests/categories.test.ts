...
test("all categories are returned", async () => {
  const response = await api.get(process.env.TEST_API_URL + "/categories").send();
...
test('add category, then all categories and new type are returned', async () => {
    await api.post(process.env.TEST_API_URL + '/categories') 
...
test('update name of category, then new name of category are returned', async () => {
    const res = await api.get(process.env.TEST_API_URL + "/categories").send();
    await api.put(process.env.TEST_API_URL + '/categories/'+res.body[1]._id) 
    .set('Authorization', `Bearer ${token}`)
    .send({
        name: "test2updated",
        icon:'',
...
test('delete category, then all categories less one are returned', async () => {
    const res = await api.get(process.env.TEST_API_URL + "/categories").send();
    await api.delete(process.env.TEST_API_URL + '/categories/'+res.body[1]._id) 
    .set('Authorization', `Bearer ${token}`)
    .expect(200)
    const response = await api.get(process.env.TEST_API_URL + "/categories").send();
    console.log(response.body);
...
test('wrong id return status 500, and without modification', async () => {
    await api.delete(process.env.TEST_API_URL + '/categories/134567') 
    .set('Authorization', `Bearer ${token}`)
    .expect(500)
    const response = await api.get(process.env.TEST_API_URL + "/categories").send();
    console.log(response.body);
